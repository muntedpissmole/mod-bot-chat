...
static BotChatEvents eventChatter;
static std::unordered_map<Player*, std::chrono::steady_clock::time_point> botEventCooldowns;

void BotChatEvents::DispatchGameEvent(Player* source, std::string type, std::string detail)
{
    ...
    if (isSourceBot && !hasNearbyRealPlayer && !isGuildEvent)
    {
        if (g_DebugEnabled)
        {
            //LOG_INFO("server.loading", "[BotChat] Skipping bot source {} - no real players nearby", source->GetName());
        }
        return;
    }

    if (g_DebugEnabled)
        LOG_INFO("server.loading", "[BotChat] DispatchGameEvent from {} | type={} | detail={}", source->GetName(), type, detail);
    ...
    else
    {
        ...
        for (auto const& pair : source->GetMap()->GetPlayers())
        {
            ...
            if (player->IsWithinDist(source, maxDist, false)) {
                candidateBots.push_back(player);
                if (g_DebugEnabled)
                    LOG_INFO("server.loading", "[BotChat] Nearby player {} within {:.1f} yards", player->GetName(), maxDist);
            }
        }
    }
    ...
    for (Player* bot : candidateBots)
    {
        ...
        if (!ai) {
            if (g_DebugEnabled)
                LOG_INFO("server.loading", "[BotChat] Skipping {} - not a bot", bot->GetName());
            continue;
        }

        if (disableInCombat && bot->IsInCombat()) {
            if (g_DebugEnabled)
                LOG_INFO("server.loading", "[BotChat] Skipping {} - in combat", bot->GetName());
            continue;
        }
        ...
        if (urand(1, 100) > chance) {
            if (g_DebugEnabled)
                LOG_INFO("server.loading", "[BotChat] Skipping {} - failed chance roll", bot->GetName());
            continue;
        }

        if (g_DebugEnabled)
            LOG_INFO("server.loading", "[BotChat] Queueing event for bot {}", bot->GetName());
    ...
    if (g_DebugEnabled)
        LOG_INFO("server.loading", "[BotChat] Dispatch complete. {} bots responded", responses);
}


void BotChatEvents::QueueEvent(Player* bot, std::string type, std::string detail, std::string actorName, bool isGuildEvent)
{
    ...
    std::thread([this, botGuid, type, detail, actorName, isGuildEvent]()
    {
        try
        {
            ...
            std::string response;
            SocialAct eventAct = SocialAct::None;
            if (isGuildEvent && g_EnableSocialConventions)
            {
                if (type == g_GuildEventTypeEpicGear || type == g_GuildEventTypeRareGear ||
                    type == g_GuildEventTypeLevelUp || type == g_EventTypeLeveledUp ||
                    type == g_GuildEventTypeGuildAchievement || type == g_EventTypeAchievement)
                    eventAct = SocialAct::Congrats;
                else if (type == g_GuildEventTypeGuildLogin)
                    eventAct = SocialAct::WelcomeBack;
                else if (type == g_GuildEventTypeGuildJoin)
                    eventAct = SocialAct::Welcome;
            }

            std::string eventThreadKey = MakeThreadKey(botPtr,
                isGuildEvent ? SRC_GUILD_LOCAL : SRC_SAY_LOCAL, nullptr);
            if (eventAct != SocialAct::None)
                response = PickSocialComment(eventAct, eventThreadKey);
            else if (type == g_EventTypeCompletedQuest || type == g_EventTypeLeveledUp ||
                     type == g_EventTypeAchievement)
                response = PickSocialComment(SocialAct::Congrats, eventThreadKey);
            else if (type == g_EventTypeDied || type == g_EventTypeDefeatedPlayer)
                response = PickSocialComment(SocialAct::Condolence, eventThreadKey);
            else if (type == g_EventTypeWonDuel)
                response = PickSocialComment(SocialAct::GoodGame, eventThreadKey);
            else
            {
                // Grind kills, trash loot, learned spell, used object: stay quiet.
                // The 3b model turns those into sentences about the item.
                return;
            }
            if (!response.empty() && LineTooSimilarToRecent(eventThreadKey, response))
                response.clear();
            ...
            if (response.empty())
            {
                if (g_DebugEnabled)
                    LOG_INFO("server.loading",
                        "[BotChat] Bot {} skipped event response (empty, slang-salad, or repeat)",
                        botPtr->GetName());
                return;
            }
            ...
            if (isGuildEvent && botPtr->GetGuild())
            {
                ...
                {
                    if (g_DebugEnabled)
                        LOG_INFO("server.loading", "[Bot Chat] Guild event chatter skipped (guild channels disabled)");
                    return;
                }
                
                botAI->SayToGuild(response);
                AppendChannelThread(MakeThreadKey(botPtr, SRC_GUILD_LOCAL, nullptr),
            ...
            else if (botPtr->GetGroup())
            {
                ...
                {
                    if (g_DebugEnabled)
                        LOG_INFO("server.loading", "[Bot Chat] Party event chatter skipped (party channels disabled)");
                    return;
                }
                
                botAI->SayToParty(response);
                AppendChannelThread(MakeThreadKey(botPtr, SRC_PARTY_LOCAL, nullptr),
                    botPtr->GetName(), botPtr->GetGUID().GetRawValue(), true, response);
            }
            else
            {
                // Do not dump gz into General. Say only if a real player can hear it.
                if (g_DisableForSayYell)
                    return;
                bool realPlayerInSayDistance = false;
                for (auto const& pair : ObjectAccessor::GetPlayers())
                {
                    Player* listener = pair.second;
                    if (!listener || !listener->IsInWorld())
                        continue;
                    if (PlayerbotsMgr::instance().GetPlayerbotAI(listener))
                        continue;
                    if (botPtr->GetDistance(listener) <= g_SayDistance)
                    {
                ...
                if (!realPlayerInSayDistance)
                    return;
                if (g_DebugEnabled)
                    LOG_INFO("server.loading", "[Bot Chat] Bot Event Chatter Say: {}", response);
                botAI->Say(response);
                AppendChannelThread(MakeThreadKey(botPtr, SRC_SAY_LOCAL, nullptr),
                    botPtr->GetName(), botPtr->GetGUID().GetRawValue(), true, response);
            }
        }
        catch (const std::exception& e)
        {
            LOG_ERROR("server.loading", "[BotChat] Exception in QueueEvent thread: {}", e.what());
        }
    }).detach();
}


std::string BotChatEvents::BuildPrompt(Player* bot, std::string promptTemplate, std::string eventType, std::string eventDetail, std::string actorName)
{
    if (!bot) return "";
...
void ChatOnLoot::OnPlayerStoreNewItem(Player* player, Item* item, uint32 /*count*/)
{
    ...
    bool realGear = proto->Class == ITEM_CLASS_WEAPON || proto->Class == ITEM_CLASS_ARMOR;

    if (proto->Quality >= ITEM_QUALITY_UNCOMMON && realGear)
    {
        eventChatter.DispatchGameEvent(player, g_EventTypeGotItem, proto->Name1);
        if (proto->Quality >= ITEM_QUALITY_RARE)
            NoteSocialCue(player, SocialAct::Congrats);
    }

    // Guild: only real weapons/armor. Hunter ammo like Shatter Rounds is epic but not gear.
    if (player->GetGuild() && g_EnableGuildEventChatter && realGear)
    {
        if (proto->Quality == ITEM_QUALITY_EPIC && !g_GuildEventTypeEpicGear.empty())
        {
            eventChatter.DispatchGameEvent(player, g_GuildEventTypeEpicGear, proto->Name1);
            NoteSocialCue(player, SocialAct::Congrats);
        }
        else if (proto->Quality == ITEM_QUALITY_RARE && !g_GuildEventTypeRareGear.empty())
        {
            eventChatter.DispatchGameEvent(player, g_GuildEventTypeRareGear, proto->Name1);
        }
    }
}
...
void ChatOnDeath::OnPlayerJustDied(Player* player)
{
    ...
    NoteSocialCue(player, SocialAct::Condolence);
}
...
void ChatOnDuel::OnPlayerDuelEnd(Player* winner, Player* loser, DuelCompleteType /*type*/)
{
    ...
    NoteSocialCue(winner, SocialAct::Congrats);
}
...
void ChatOnGuildMemberChange::OnGuildMemberJoin(Player* player, Guild* /*guild*/)
{
    ...
    if (!g_GuildEventTypeGuildJoin.empty())
    {
        ...
        NoteSocialCue(player, SocialAct::Welcome);
    }
}
...
void ChatOnGuildMemberChange::OnGuildMemberLogin(Player* player, Guild* guild)
{
    ...
    if (!g_GuildEventTypeGuildLogin.empty())
    {
        ...
        NoteSocialCue(player, SocialAct::WelcomeBack);
    }
}
